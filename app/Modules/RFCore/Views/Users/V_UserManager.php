			...
			"columnDefs": [
				{
					...
					'render': function (data, type, full, meta) {
						...
						if ((<?= session()->get('id'); ?> !== full['id'])) {
							if (!(parseInt(full['roles']) & <?= ROLE_ADMIN ?>))
							{
								...
								<?php if (session()->get('roles') & ROLE_ADMIN): ?>
								ret += ' <a title="Se connecter en tant que" class="btn btn-success" href="<?= base_url('Users/loginAs') ?>/'+full['id']+'"><i class="fas fa-sign-in-alt text-white"></i></a>';
								<?php endif; ?>
							}
						}
						else {
	...
	function deleteUser(email,id) {
		...
		if (answer) {
			...
			$.ajax({
				...
				error: function (data) {
					displayToast('Erreur', 'Une erreur interne est survenue lors du traitement de votre requête', <?= TOAST_ERROR ?>);
				}
			});
		}
	}
