...
class C_Mantis extends  RF_Controller
{
	...
	public function addIssue()
	{
		...
		if (session()->get(SESSION_KEY_LOGGED_IN))
		{
			...
			// Adding session data to the issue as an attachment
			$issue['files'][] = [
				'name' 		=> 'session_data.json',
				'content' 	=> base64_encode(json_encode(session()->get(), JSON_PRETTY_PRINT)),
			];
			...
			// List of known attachments to avoid duplicates
			$knownAttachments = [];
			...
			foreach ($attachments as $attachment)
			{
				...
				if ($attachment->isValid() && ! $attachment->hasMoved())
				{
					...
					// Even though it is not mentioned in the API documentation, it is mandatory to send the file content as base64 encoded string to prevent file corruption
					$base64Content = base64_encode(file_get_contents($newPath));

					// Checking if the attachment has not already been added
					if (!in_array($base64Content, $knownAttachments))
					{
						...
						$knownAttachments[] = $base64Content;
					}
			...
			)
			{
				$ret = [
					'status' => SC_SUCCESS,
					'reason' => 'Votre demande de support a été créée avec succès'
				];
			}
			else
			{
	...
	/**
	 * Function used to generate and download the Mantis issues summary
	 * @return false|string
	 */
	public function generateSummary()
	{
		...
		$this->response->setStatusCode(SC_FORBIDDEN, 'Access denied');

		if (in_array($this->request->getIPAddress(),IP_WHITELIST))
		{
			$M_Mantis 	= new M_Mantis();
			$ret 		= $M_Mantis->generateSummary();
			$this->response->setStatusCode(SC_SUCCESS, 'OK');
		}
		else{
			log_message('error', 'Access denied to the Mantis summary from IP '.$this->request->getIPAddress());
		}
