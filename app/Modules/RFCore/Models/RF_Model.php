...
class RF_Model extends Model
{
    ...
    public function __construct(ConnectionInterface &$db = null, ValidationInterface $validation = null, $dbName = null)
    {
        $this->dbName = $dbName;
	...
	/**
	 * Function responsible for encoding an array in UTF-8 recursively
	 * @param $array array Array to encode
	 * @return array Encoded array
	 */
	public function utf8_encode_array(array &$array): array
	{
		try {
			array_walk_recursive($array, function(&$item, $key){
				if(is_string($item)){
					$array[$key] = utf8_encode($item);
				}
			});
		} catch (Exception $e) {
			log_message('error', __CLASS__ . '::' . __FUNCTION__ . ' : ' . $e);
