...
class M_User extends RF_Model
{
	...
	/**
	 * Set up the current session data based on the given user ID
	 * @param string $uid User ID
	 * @param bool $regenerateSession Does the session need to be regenerated ?
	 * @return array
	 */
	public function setUserdata(string $uid, bool $regenerateSession = false): array
	{
		...
		try {
			...
			{
				// Regenerate a clean session in order to remove any existing session data
				session()->regenerate(true);
			}

			// Retrieving the user data based on the given ID
			$user = $this->findOneBy('id', $uid);
			...
			if (!empty($user))
			{
				...
				// Adding the additional data to the session
				$additionalData = $user->getAdditionalData(true);
				...
				$ret['reason'] = 'Utilisateur introuvable';
			}
		}
		catch (Exception $e) {
	...
	public function refreshLoggedUserSessionData(): int
	{
		...
		if (!empty($loggedUser))
		{
			...
			// Defining session data for the logged user
			$userData = $loggedUser->getProperties();
        ...
        if (COMFOX_AVAILABLE)
        {
            ...
            try
            {
                ...
                if (($user != NULL))
                {
					...
					$data[emailSubject] = utf8_decode(INTEGRATION_NEW_PWD_EMAIL_TITLE);
					$data[emailMessage] = utf8_decode(view(INTEGRATION_NEW_PWD_EMAIL_MESSAGE, ['link' => $htmlLink, 'email' => $user->getEmail()]));
        ...
        if (COMFOX_AVAILABLE)
        {
            ...
            try {
				...
				if (($user != NULL)) {
					...
					$from = $data[emailConfig][emailFrom];
					if (!empty($from)) {
						$data[emailFrom] = $from;
					}
					...
					if (!$firstConnection) {
						...
						$data[emailSubject] = utf8_decode(INTEGRATION_UPDATE_PWD_EMAIL_TITLE);
						$data[emailMessage] = utf8_decode(view(INTEGRATION_UPDATE_PWD_EMAIL_MESSAGE, ['link' => $htmlLink, 'email' => $user->getEmail()]));
					} else {
						// User logs in for the first time
						// Generate an URL containing the required parameters (token + process step)
