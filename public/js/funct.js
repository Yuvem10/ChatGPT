...
function displayToast(title,message,type,delay = 0,icon = null)
{
    ...
    // Duplicating the current toast index to prevent it from being modified by parallel requests
    let currentToastIndex = toastIndex;
    ...
    $.ajax({
        ...
        success     : function (ret) {
            $('#toast-container').append(ret);
            initToast(currentToastIndex, delay, 0);
        }
    })
}

/**
 * Initialize a toast element
 * @param currentToastIndex Index of the toast element
 * @param delay Delay before hiding the toast element (0 = no auto hiding)
 * @param loopCount Number of times the function has been called
 */
function initToast(currentToastIndex, delay, loopCount = 0)
{
    let newToast    = $('#toast'+currentToastIndex);

    // Making sure the element exists
    if (newToast.length !== 0) {
        newToast        = bootstrap.Toast.getOrCreateInstance(newToast, {
            animation   : true,
            autohide    : (delay !== 0),
            delay       : delay
        });
    ...
    // If the element does not exist, we wait 100ms and try again
    else if(loopCount < 10) {
        loopCount++;
        setTimeout(function() {
            initToast(currentToastIndex, delay, loopCount);
        }, 100);
    }
    else {
        console.log('Toast "toast'+currentToastIndex+'" not found');
    }
}
...
/**
 * Check if a value is considered as empty
 * A value is considered as empty if it is null, undefined or an empty string
 * @param value
 * @returns {boolean}
 */
